...
#include <stdint.h>
#include <time.h>
#include <stm32f3xx_hal.h> // for HAL_GetTick()
...
#include <time.h>
static inline uint32_t tamper_get_tick(void) {
    return (uint32_t)(time(NULL) * 1000); // convert to ms
}
#endif
      ...
      RTC_TimeTypeDef sTime;
        ...
        return (uint32_t)(sTime.Hours * 3600000 + sTime.Minutes * 60000 + sTime.Seconds * 1000 + sTime.SubSeconds);
#else
        return (uint32_t)(time(NULL) * 1000); // convert to ms
#endif
